...
std::string savethemefilename() {
    ...
    ofn.hwndOwner = NULL;
    ofn.lpstrFilter = (LPCWSTR)L"guistyle Files (*.guistyle)\0*.guistyle\0";
    ofn.lpstrFile = (LPWSTR)fileName;
    ofn.nMaxFile = MAX_PATH;
    ofn.Flags = OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_HIDEREADONLY;
    ofn.lpstrDefExt = (LPCWSTR)L"guistyle";

    GetSaveFileName(&ofn);

    std::string fileNameStr(CW2A(ofn.lpstrFile));
    return fileNameStr;
}
namespace phx {
