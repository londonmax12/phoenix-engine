...
namespace phx {
	...
	namespace Utils {
		...
		static GLenum ShaderTypeFromString(const std::string& type)
		{
			...
			PHX_CORE_ASSERT(false, "Unknown shader type!");
			return 0;
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			PHX_CORE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1; //Start of shader type name (after "#type " keyword)
			std::string type = source.substr(begin, eol - begin);
			PHX_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");

			size_t nextLinePos = source.find_first_not_of("\r\n", eol); //Start of shader code after shader type declaration line
			PHX_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					PHX_CORE_ERROR(module.GetErrorMessage());
					PHX_CORE_ASSERT(false);
				}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			PHX_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		PHX_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		PHX_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		PHX_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		PHX_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			PHX_CORE_TRACE("  {0}", resource.name);
			PHX_CORE_TRACE("    Size = {0}", bufferSize);
			PHX_CORE_TRACE("    Binding = {0}", binding);
			PHX_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
