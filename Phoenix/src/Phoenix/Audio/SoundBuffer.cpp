...
namespace phx {
	SoundBuffer* SoundBuffer::New()
	{
		static SoundBuffer* soundBuffer = new SoundBuffer();
		return soundBuffer;
	}
	ALuint SoundBuffer::AddSoundEffect(std::filesystem::path filename)
	{
		ALenum err, format;
		ALuint buffer;
		SNDFILE* sndfile;
		...
		if (!sndfile)
		{
			PHX_CORE_INFO("Could not open audio in {0}: {1}", filename, sf_strerror(sndfile));
			return 0;
		}
		if (sfinfo.frames < 1 || sfinfo.frames >(sf_count_t)(INT_MAX / sizeof(short)) / sfinfo.channels)
		{
			PHX_CORE_INFO("Bad sample count in {0} ({1})", filename, sfinfo.frames);
			sf_close(sndfile);
			return 0;
		}
		...
		if (!format)
		{
			PHX_CORE_INFO("Unsupported channel count: {0}", sfinfo.channels);
			sf_close(sndfile);
			return 0;
		}
		...
		if (num_frames < 1)
		{
			free(membuf);
			sf_close(sndfile);
			PHX_CORE_INFO("Failed to read samples in {0} ({1})", filename, num_frames);
			return 0;
		}
		...
		if (err != AL_NO_ERROR)
		{
			PHX_CORE_INFO("OpenAL Error: {0}", alGetString(err));
